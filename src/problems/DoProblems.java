...
public class DoProblems {
    ...
    public String problemEleven() {
        Integer[][] gridOfNumbers = new Integer[20][20];
        StringTokenizer tok;
        String line = "";
        Integer[] consecInts = new Integer[4];
        int largestProduct = 0;
        try {
            ...
            for (int i = 0; i < 20; i++) {
                ...
                for (int j = 0; j < 20; j++) {
                    gridOfNumbers[i][j] = Integer.parseInt(tok.nextToken());
                }
            }
            fileReader.close();
            bufferedReader.close();
        ...
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 20; j++) {
                for (int k = j; k + 4 < 20 && k < consecInts.length; k++) {
                    consecInts[k] = gridOfNumbers[i][j];
                }
                int temp1;
                int temp2;
                if ((temp1 = consecInts[0] * consecInts[1] * consecInts[2] * consecInts[3])
                        > (temp2 = consecInts[0] * consecInts[1] * consecInts[2] * consecInts[3])) {
                    if (temp1 > largestProduct) {
                        largestProduct = temp1;
                    }
                } else {
                    if (temp2 > largestProduct) {
                        largestProduct = temp2;
                    }
                }
            }
        }
        ...
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 20; j++) {
                for (int k = j; k + 4 < 20 && k < consecInts.length; k++) {
                    consecInts[k] = gridOfNumbers[j][i];
                }
                int temp1;
                int temp2;
                if ((temp1 = consecInts[0] * consecInts[1] * consecInts[2] * consecInts[3])
